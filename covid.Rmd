...
Read confirmed_cases_worldwide.csv into confirmed_cases_worldwide and show the data

```{r confirmed_cases_worldwide , echo=TRUE}
confirmed_cases_worldwide<- read.csv(choose.files(), header = T, stringsAsFactors = T)
head(confirmed_cases_worldwide,5)
```

```{r , echo=TRUE}
confirmed_cases_worldwide<- mutate(confirmed_cases_worldwide, date=ymd(date))
...
```{r , echo=TRUE}
plt_confirmed_cases_worldwide<- ggplot(confirmed_cases_worldwide,aes(x=date,y=cum_cases,group=1)) +
                                 geom_line() + scale_x_date(date_labels = "%d-%B")
...
```{r confirmed_cases_china_vs_world , echo=TRUE}
confirmed_cases_china_vs_world<- read.csv(choose.files(), header = T, stringsAsFactors = T)
head(confirmed_cases_china_vs_world,5)
```

```{r , echo=TRUE}
confirmed_cases_china_vs_world <- mutate(confirmed_cases_china_vs_world , date=ymd(date))
```
...
plt_cum_confirmed_cases_china_vs_world +
  geom_vline(aes(xintercept = as.Date(date) ), data = who_events, linetype = "dashed") +
  geom_text(aes(as.Date(date) , label = event), data = who_events, y = 1e5)
```
...
head(china_after_feb15,5) 
```


```{r , echo=TRUE}

plt_china_after_feb15<- ggplot(china_after_feb15, aes(date, cum_cases)) +
  geom_line() +
...
```{r confirmed_cases_by_country , echo=TRUE}
confirmed_cases_by_country <- read.csv(choose.files(),header = T, stringsAsFactors = T) 
head(confirmed_cases_by_country,5)
...
top_countries_by_total_cases<- confirmed_cases_by_country %>% 
                               ...
                               top_n(7, total_cases) 
head(top_countries_by_total_cases,5)
...
```{r confirmed_cases_top7_outside_china , echo=TRUE}
confirmed_cases_top7_outside_china<- read.csv(choose.files(),header = T, stringsAsFactors = T)
head(confirmed_cases_top7_outside_china, 5)

```

```{r , echo=TRUE}
confirmed_cases_top7_outside_china <- mutate(confirmed_cases_top7_outside_china , date=ymd(date))
```

```{r  , echo=TRUE}
plt_confirmed_cases_top7_outside_china <- ggplot(confirmed_cases_top7_outside_china) +
